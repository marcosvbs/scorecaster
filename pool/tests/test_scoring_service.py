...
def test_round_not_closed_while_matches_pending(make_match, users):
    now = timezone.now()
...
def test_round_closes_when_last_match_scored(make_match, users):
    now = timezone.now()
    first = make_match(starts_at=now, round="Group Stage - 1")
    second = make_match(
    ...
    winners = RoundWinner.objects.filter(round="Group Stage - 1")
    assert winners.count() == 1
    assert winners.first().user == users[0]
    assert winners.first().points == 10


def test_close_round_if_complete_noop_for_unknown_round(db):
    assert close_round_if_complete("Nope") is None


def test_winner_tiebreak_by_total_points(make_match, users):
    """Tied in the round -> higher cumulative total wins (spec 7.2)."""
    now = timezone.now()
    # Round 1: ana gains 10, bruno 0 -> ana leads the overall ranking.
    r1 = make_match(starts_at=now - timezone.timedelta(days=3), round="Group Stage - 1")
    ...
    # Round 2: both score 5 (hit winner only) -> tie on round points.
    r2 = make_match(starts_at=now, round="Group Stage - 2")
    ...
    winners = RoundWinner.objects.filter(round="Group Stage - 2")
    assert winners.count() == 1
    assert winners.first().user == users[0]
...
def test_no_winners_without_predictions(make_match):
    ...
    assert compute_round_winners("Group Stage - 1") == []
    assert RoundWinner.objects.count() == 0
...
def test_admin_correction_updates_winner(make_match, users):
    ...
    finish(match, 2, 0)
    assert RoundWinner.objects.get(round="Group Stage - 1").user == users[0]
